...
<section aria-labelledby="insights-heading"><p class="eyebrow">Descriptive analysis only</p><h1 id="insights-heading">Rule outcomes and synthesis</h1><p class="disclosure"><strong>Not investment advice.</strong> These deterministic observations are descriptive, are not personalized, and do not provide a recommendation, target price or confidence score.</p><p>Shared analysis fingerprint: <code>{fingerprint}</code></p>{#each rules as rule (rule.id)}<InsightRuleResult ruleId={rule.id} title={rule.title} outcome={rule.outcome} explanation={rule.explanation} {fingerprint} />{/each}<aside aria-labelledby="synthesis-heading"><h2 id="synthesis-heading">Synthesis: mixed with limitations</h2><p>Several operating and cash-flow signals are positive, while leverage and return-on-equity remain not evaluable.</p><h3>Limitations</h3><ul><li>Debt classification is incomplete.</li><li>Non-positive equity prevents one ratio from being evaluated.</li><li>Price data is intentionally excluded.</li></ul></aside></section>
<style>section{max-inline-size:70rem}.eyebrow{font-weight:700;text-transform:uppercase}.disclosure,aside{border-inline-start:.3rem solid currentColor;padding-inline-start:1rem}</style>
