...
const operation=handoff.operation??{id:handoff.bootstrap?.id??state.activeBatchId,kind:'BOOTSTRAP',branch:handoff.bootstrap?.branch??branch,stage:handoff.bootstrap?.stage??'candidate',activeBatchId:state.activeBatchId};
const batchId=operation.activeBatchId??state.activeBatchId; const batch=await readJson(join(root,`implementation-control/batches/${batchId}.json`));
const isOperationBranch=branch===operation.branch; const stage=operation.stage??'candidate'; const releaseRemediation=isOperationBranch&&operation.kind==='RELEASE_REMEDIATION'; const releaseAudit=isOperationBranch&&operation.kind==='RELEASE_AUDIT'; const usesReleaseCommands=releaseRemediation||releaseAudit;
const mode=usesReleaseCommands?'RELEASE_REMEDIATION':isOperationBranch&&stage==='closure'?'BATCH_CLOSURE':isOperationBranch&&operation.kind==='BOOTSTRAP'?'GH0_BOOTSTRAP':'BATCH';
...
const releaseQualificationCommands=(operation.qualificationCommands??[]).map(({id,category,command,required})=>({id,category,command,required:Boolean(required)}));
const selectedCommands=mode==='BATCH_CLOSURE'?[]:usesReleaseCommands?releaseQualificationCommands:derivedBatchCommands;
const declaredCommands=usesReleaseCommands?releaseQualificationCommands:derivedBatchCommands;
...
else if(usesReleaseCommands&&releaseQualificationCommands.length===0) fail('RELEASE_QUALIFICATION_COMMANDS_EMPTY','No release qualification commands were declared.');
