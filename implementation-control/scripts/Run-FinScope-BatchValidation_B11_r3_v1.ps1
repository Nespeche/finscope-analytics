...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$WorkRoot = 'C:\FS\B11r3v1\work',
...
$CandidateLogicalName = 'FS_B11_r3.zip'
$CandidateSidecarLogicalName = 'FS_B11_r3.zip.sha256'
$RunnerLogicalName = 'Run-FinScope-BatchValidation_B11_r3_v1.ps1'
$RunnerSidecarLogicalName = 'Run-FinScope-BatchValidation_B11_r3_v1.ps1.sha256'
$ExpectedRootDirectory = 'FinScope_v0.21.4'
$ExpectedTasksSha256 = '57e9167d5cd0e22265a4fcfd1d8697cc0233aaf8fb9fc4a79442da962db855a8'
$ExpectedSpecifyTreeSha256 = 'e06c8fbab523b824c144bb22b616001a3a4e810bb9daaa793e84d5cbb77c2c09'
$ExpectedSourceBaselineSha256 = '8a73e0ebbb8bb4e56f3aeb1df7982ae8bbd9e4789060d830250985820d86c06e'
$PackageIntegrityExcludedPaths = [string[]]@('FILE_MANIFEST.sha256', 'PACKAGE_INVENTORY.json')
...
function Assert-ShortWorkRoot {
    ...
    if ($fullPath -notmatch '^[A-Za-z]:\\FS\\[^\\]+\\work(?:\\)?$') {
        throw "WORK_ROOT_NOT_SHORT_OR_UNSAFE: use a path like C:\FS\B11r3v1\work; actual=${fullPath}"
    }
...
function Assert-FileManifest {
    ...
    $actual = @(Get-RelativeFileRecords $ProjectRoot | Where-Object {
        $_.RelativePath -notin $PackageIntegrityExcludedPaths
    })
...
function Assert-ExplicitGateAnchor {
    param([Parameter(Mandatory = $true)][string]$ProjectRoot)
    $relativePath = 'V0.21_PHASE_STATUS.md'
    $phaseStatusPath = Join-Path $ProjectRoot $relativePath
    if (-not (Test-Path -LiteralPath $phaseStatusPath -PathType Leaf)) {
        throw "EXPLICIT_GATE_ANCHOR_DOCUMENT_MISSING: ${relativePath}"
    }
    $expectedAnchor = '<a id="gate"></a>'
    $content = Get-Content -LiteralPath $phaseStatusPath -Raw -Encoding UTF8
    $anchorCount = [regex]::Matches($content, [regex]::Escape($expectedAnchor)).Count
    if ($anchorCount -ne 1) {
        throw "EXPLICIT_GATE_ANCHOR_INVALID: expected=1; actual=${anchorCount}; anchor=${expectedAnchor}"
    }
    return [ordered]@{
        status = 'PASS'
        ...
        count = $anchorCount
        sha256 = Get-Sha256 $phaseStatusPath
    }
}
...
function Assert-ControlState {
    ...
    $batchPath = Join-Path $ProjectRoot 'implementation-control\batches\B11.json'
    ...
    if ([string]$batch.status -ne 'LOCAL_VALIDATION_REQUIRED') { throw 'B11_BATCH_FILE_STATUS_MISMATCH' }
    ...
    if ($lockEntry.Count -ne 1 -or [string]$lockEntry[0].sha256 -ne $batchActualHash) {
        throw "TASK_MIRROR_MISMATCH: B11 batch lock does not match ${batchActualHash}"
    }
...
function Invoke-FileManifestExclusionSelfTests {
    $manifestRoot = Join-Path ([System.IO.Path]::GetTempPath()) "FinScope-B11-manifest-selftest-$([Guid]::NewGuid().ToString('N'))"
    New-Item -ItemType Directory -Path $manifestRoot -Force | Out-Null
    try {
        $uniqueExclusions = @($PackageIntegrityExcludedPaths | Sort-Object -Unique)
        if (
            $PackageIntegrityExcludedPaths.Count -ne 2 -or
            $uniqueExclusions.Count -ne 2 -or
            'FILE_MANIFEST.sha256' -notin $PackageIntegrityExcludedPaths -or
            'PACKAGE_INVENTORY.json' -notin $PackageIntegrityExcludedPaths
        ) {
            throw 'SELF_TEST_PACKAGE_INTEGRITY_EXCLUSION_SET_INVALID'
        }

        $payloadPath = Join-Path $manifestRoot 'payload.txt'
        $inventoryPath = Join-Path $manifestRoot 'PACKAGE_INVENTORY.json'
        $manifestPath = Join-Path $manifestRoot 'FILE_MANIFEST.sha256'
        [System.IO.File]::WriteAllText($payloadPath, 'payload', $Utf8NoBom)
        [System.IO.File]::WriteAllText($inventoryPath, '{}', $Utf8NoBom)
        [System.IO.File]::WriteAllText(
            $manifestPath,
            ((Get-Sha256 $payloadPath) + '  payload.txt' + [Environment]::NewLine),
            $Utf8NoBom
        )

        [void](Assert-FileManifest $manifestRoot)
        $declaredPayloadCount = @(Get-RelativeFileRecords $manifestRoot | Where-Object {
            $_.RelativePath -notin $PackageIntegrityExcludedPaths
        }).Count
        if ($declaredPayloadCount -ne 1) {
            throw "SELF_TEST_FILE_MANIFEST_VALID_COUNT_FAILED: actual=${declaredPayloadCount}"
        }

        $extraPath = Join-Path $manifestRoot 'undeclared.txt'
        [System.IO.File]::WriteAllText($extraPath, 'undeclared', $Utf8NoBom)
        $extraFileNegative = Assert-ExpectedFailure `
            -CaseId 'manifest-undeclared-file' `
            -ExpectedPattern 'FILE_MANIFEST_COUNT_MISMATCH' `
            -Action { [void](Assert-FileManifest $manifestRoot) }
        Remove-Item -LiteralPath $extraPath -Force

        [System.IO.File]::WriteAllText($payloadPath, 'tampered', $Utf8NoBom)
        $hashNegative = Assert-ExpectedFailure `
            -CaseId 'manifest-payload-hash' `
            -ExpectedPattern 'FILE_MANIFEST_HASH_MISMATCH' `
            -Action { [void](Assert-FileManifest $manifestRoot) }

        return [ordered]@{
            status='PASS'
            exclusions=$PackageIntegrityExcludedPaths
            validPayloadCount=$declaredPayloadCount
            undeclaredFileNegative=$extraFileNegative
            hashNegative=$hashNegative
        }
    }
    finally {
...
function Invoke-RunnerQualificationSelfTests {
    ...
    $selfRoot = Join-Path ([System.IO.Path]::GetTempPath()) "FinScope-B11-runner-selftest-$([Guid]::NewGuid().ToString('N'))"
    New-Item -ItemType Directory -Path $selfRoot -Force | Out-Null
    try {
        $valueShapes = Invoke-ValueShapeSelfTests
        $fileManifestExclusions = Invoke-FileManifestExclusionSelfTests

        $gateRoot = Join-Path $selfRoot 'gate-anchor'
        New-Item -ItemType Directory -Path $gateRoot -Force | Out-Null
        $gatePath = Join-Path $gateRoot 'V0.21_PHASE_STATUS.md'
        [System.IO.File]::WriteAllText($gatePath, ("# Gate`n`n<a id=`"gate`"></a>`n"), $Utf8NoBom)
        $gateAnchorValid = Assert-ExplicitGateAnchor $gateRoot
        [System.IO.File]::WriteAllText($gatePath, "# Gate without explicit anchor`n", $Utf8NoBom)
        $gateAnchorNegative = Assert-ExpectedFailure `
            -CaseId 'explicit-gate-anchor-missing' `
            -ExpectedPattern 'EXPLICIT_GATE_ANCHOR_INVALID' `
            -Action { [void](Assert-ExplicitGateAnchor $gateRoot) }

        $safeWorkRoot = Assert-ShortWorkRoot 'C:\FS\B11r3v1\work'
        ...
        $validInspection = Get-ArchiveInspection $validZip 'C:\FS\B11-selftest\work'
        ...
        $rootNegative = Assert-ExpectedFailure -CaseId 'wrong-root' -ExpectedPattern 'ARCHIVE_PREFLIGHT_FAILED' -Action { [void](Assert-ArchiveInspection (Get-ArchiveInspection $wrongRootZip 'C:\FS\B11-selftest\work')) }

        $traversalZip = New-SyntheticZip (Join-Path $selfRoot 'traversal.zip') ([ordered]@{ "${ExpectedRootDirectory}/../escape.txt"='invalid' })
        $traversalNegative = Assert-ExpectedFailure -CaseId 'traversal' -ExpectedPattern 'ARCHIVE_PREFLIGHT_FAILED' -Action { [void](Assert-ArchiveInspection (Get-ArchiveInspection $traversalZip 'C:\FS\B11-selftest\work')) }

        $evidenceRoot = Join-Path $selfRoot 'evidence'
        New-Item -ItemType Directory -Path $evidenceRoot -Force | Out-Null
        foreach ($name in @('B11-local-validation.json','VERIFICATION_SUMMARY.json')) {
        ...
        return [ordered]@{
            status='PASS'; valueShapes=$valueShapes; fileManifestExclusions=$fileManifestExclusions
            explicitGateAnchor=[ordered]@{ valid=$gateAnchorValid; missingNegative=$gateAnchorNegative }
            safeWorkRoot=$safeWorkRoot; unsafeWorkRoot=$unsafeWorkRoot
...
function New-FailureDiagnostic {
    ...
    try {
        ...
        $root = Join-Path $OutputDirectory "FinScope_runner_diagnostic_B11_${RunStamp}_FAILED"
...
try {
    ...
    if ($SelfTestOnly) {
        $selfTestPath = Join-Path $ResolvedInput "FinScope_runner_selftest_B11_${RunStamp}.json"
        ...
        Write-Host "Runner B11 r3 v1 SelfTest PASS (sin candidato ni npm): ${selfTestPath}" -ForegroundColor Green
        exit 0
    }
    ...
    $explicitGateAnchor = Assert-ExplicitGateAnchor $projectRoot
    ...
    $preflight = [ordered]@{
        ...
        hashes = [ordered]@{ tasksSha256=$tasksHash; specifyTreeSha256=$specifyHash; B11Sha256=$control.BatchHash; extractedTreeSha256=$initialTreeSha256 }
        packageHygiene = $packageHygiene
        explicitGateAnchor = $explicitGateAnchor
        authorityFiles = $authorityFiles
        executability = $executability
        state = [ordered]@{ implementationStatus=[string]$control.State.implementationStatus; activeBatchId=[string]$control.State.activeBatchId; nextAuthorizedBatchId=[string]$control.State.nextAuthorizedBatchId; B10=[string]$control.State.batchStatus.B10; B11=[string]$control.State.batchStatus.B11; B12=[string]$control.State.batchStatus.B12; convergenceAuthorized=[bool]$control.State.phaseGate.convergenceAuthorized }
    }
    $PreflightReportPath = Join-Path $ResolvedInput "FinScope_preflight_B11_${RunStamp}.json"
    ...
    Write-Host "Preflight B11 r3 v1 PASS (sin npm): ${PreflightReportPath}" -ForegroundColor Green
    if ($PreflightOnly) { exit 0 }

    $EvidenceRoot = Join-Path $resolvedWorkRoot "FinScope_local_evidence_B11_${RunStamp}"
    ...
    try {
        foreach ($entry in @($control.Batch.localValidation.commands)) {
            ...
            Write-Host "[B11] Ejecutando: $($entry.command)" -ForegroundColor Cyan
    ...
    $evidenceJsonPath = Join-Path $EvidenceRoot 'B11-local-validation.json'
    ...
    $explicitGateAnchorFinal = Assert-ExplicitGateAnchor $projectRoot
    ...
    $verificationSummary = [ordered]@{
        ...
        explicitGateAnchor=$explicitGateAnchorFinal
    ...
    $baseName = "FinScope_local_evidence_B11_${RunStamp}" + $(if ($overallPass) { '' } else { '_FAILED' })
