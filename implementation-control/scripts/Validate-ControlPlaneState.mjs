...
try {
  ...
  check('B20_CLOSURE_FORCE_WITH_LEASE', closureRequestWorkflow.includes('--force-with-lease=refs/heads/maintenance/b20-post-restore-control-plane-hardening:$EVENT_HEAD'), 'exact-head lease');
  ...
  check('B20_CLOSURE_VALIDATION_READ_ONLY', closureValidationWorkflow.includes('contents: read') && !closureValidationWorkflow.includes('contents: write'), 'closure validation permissions');
  ...
  check('B20_CLOSURE_R3_HANDOFF_IDENTITY', handoff.release?.releaseRevision === 'v0.21.25_B20_completed_r3' && handoff.release?.tag === 'v0.21.25-B20-completed-r3' && handoff.release?.zipName === 'FS_v0.21.25_B20_completed_r3.zip' && handoff.release?.sidecarName === 'FS_v0.21.25_B20_completed_r3.zip.sha256', JSON.stringify(handoff.release));
  check('B20_CLOSURE_R3_STATE_IDENTITY', state.activePackageLogicalName === 'FS_v0.21.25_B20_completed_r3.zip', state.activePackageLogicalName);
  check('B20_CLOSURE_R3_METADATA_IDENTITY', metadata.releaseRevision === 'v0.21.25_B20_completed_r3' && metadata.logicalZipName === 'FS_v0.21.25_B20_completed_r3.zip' && metadata.finalSha256Sidecar === 'FS_v0.21.25_B20_completed_r3.zip.sha256', JSON.stringify({ releaseRevision: metadata.releaseRevision, logicalZipName: metadata.logicalZipName }));
