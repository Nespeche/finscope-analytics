...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$WorkRoot = 'C:\FS\B10r2v1\work',
...
$CandidateLogicalName = 'FS_B10_r2.zip'
$CandidateSidecarLogicalName = 'FS_B10_r2.zip.sha256'
$RunnerLogicalName = 'Run-FinScope-BatchValidation_B10_r2_v1.ps1'
$RunnerSidecarLogicalName = 'Run-FinScope-BatchValidation_B10_r2_v1.ps1.sha256'
$ExpectedRootDirectory = 'FinScope_v0.21.4'
$ExpectedTasksSha256 = '71d17254d3d8698befc2c1f75b4619d80b0f98545560400470d452796ef6201c'
$ExpectedSpecifyTreeSha256 = 'e06c8fbab523b824c144bb22b616001a3a4e810bb9daaa793e84d5cbb77c2c09'
$ExpectedSourceBaselineSha256 = '1d24563346366a4a4e6bd3780520a0e46784e42d3ca1cf708ac555a4dec65161'
...
function Assert-ShortWorkRoot {
    ...
    if ($fullPath -notmatch '^[A-Za-z]:\\FS\\[^\\]+\\work(?:\\)?$') {
        throw "WORK_ROOT_NOT_SHORT_OR_UNSAFE: use a path like C:\FS\B10r2v1\work; actual=${fullPath}"
    }
...
function Assert-ControlState {
    ...
    $batchPath = Join-Path $ProjectRoot 'implementation-control\batches\B10.json'
    ...
    if ([string]$batch.status -ne 'LOCAL_VALIDATION_REQUIRED') { throw 'B10_BATCH_FILE_STATUS_MISMATCH' }
    ...
    if ($lockEntry.Count -ne 1 -or [string]$lockEntry[0].sha256 -ne $batchActualHash) {
        throw "TASK_MIRROR_MISMATCH: B10 batch lock does not match ${batchActualHash}"
    }
...
function Invoke-FileManifestExclusionSelfTests {
    $manifestRoot = Join-Path ([System.IO.Path]::GetTempPath()) "FinScope-B10-manifest-selftest-$([Guid]::NewGuid().ToString('N'))"
...
function Invoke-RunnerQualificationSelfTests {
    ...
    $selfRoot = Join-Path ([System.IO.Path]::GetTempPath()) "FinScope-B10-runner-selftest-$([Guid]::NewGuid().ToString('N'))"
    ...
    try {
        ...
        $safeWorkRoot = Assert-ShortWorkRoot 'C:\FS\B10r2v1\work'
        ...
        $validInspection = Get-ArchiveInspection $validZip 'C:\FS\B10-selftest\work'
        ...
        $rootNegative = Assert-ExpectedFailure -CaseId 'wrong-root' -ExpectedPattern 'ARCHIVE_PREFLIGHT_FAILED' -Action { [void](Assert-ArchiveInspection (Get-ArchiveInspection $wrongRootZip 'C:\FS\B10-selftest\work')) }

        $traversalZip = New-SyntheticZip (Join-Path $selfRoot 'traversal.zip') ([ordered]@{ "${ExpectedRootDirectory}/../escape.txt"='invalid' })
        $traversalNegative = Assert-ExpectedFailure -CaseId 'traversal' -ExpectedPattern 'ARCHIVE_PREFLIGHT_FAILED' -Action { [void](Assert-ArchiveInspection (Get-ArchiveInspection $traversalZip 'C:\FS\B10-selftest\work')) }

        $evidenceRoot = Join-Path $selfRoot 'evidence'
        New-Item -ItemType Directory -Path $evidenceRoot -Force | Out-Null
        foreach ($name in @('B10-local-validation.json','VERIFICATION_SUMMARY.json')) {
        ...
        return [ordered]@{
            status='PASS'; valueShapes=$valueShapes; fileManifestExclusions=$fileManifestExclusions; safeWorkRoot=$safeWorkRoot; unsafeWorkRoot=$unsafeWorkRoot
...
function New-FailureDiagnostic {
    ...
    try {
        ...
        $root = Join-Path $OutputDirectory "FinScope_runner_diagnostic_B10_${RunStamp}_FAILED"
...
try {
    ...
    if ($SelfTestOnly) {
        $selfTestPath = Join-Path $ResolvedInput "FinScope_runner_selftest_B10_${RunStamp}.json"
        ...
        Write-Host "Runner B10 r2 v1 SelfTest PASS (sin candidato ni npm): ${selfTestPath}" -ForegroundColor Green
        exit 0
    }
    ...
    $preflight = [ordered]@{
        ...
        hashes = [ordered]@{ tasksSha256=$tasksHash; specifyTreeSha256=$specifyHash; B10Sha256=$control.BatchHash; extractedTreeSha256=$initialTreeSha256 }
        packageHygiene = $packageHygiene
        authorityFiles = $authorityFiles
        executability = $executability
        state = [ordered]@{ implementationStatus=[string]$control.State.implementationStatus; activeBatchId=[string]$control.State.activeBatchId; nextAuthorizedBatchId=[string]$control.State.nextAuthorizedBatchId; B09=[string]$control.State.batchStatus.B09; B10=[string]$control.State.batchStatus.B10; B11=[string]$control.State.batchStatus.B11; convergenceAuthorized=[bool]$control.State.phaseGate.convergenceAuthorized }
    }
    $PreflightReportPath = Join-Path $ResolvedInput "FinScope_preflight_B10_${RunStamp}.json"
    ...
    Write-Host "Preflight B10 r2 v1 PASS (sin npm): ${PreflightReportPath}" -ForegroundColor Green
    if ($PreflightOnly) { exit 0 }

    $EvidenceRoot = Join-Path $resolvedWorkRoot "FinScope_local_evidence_B10_${RunStamp}"
    ...
    try {
        foreach ($entry in @($control.Batch.localValidation.commands)) {
            ...
            Write-Host "[B10] Ejecutando: $($entry.command)" -ForegroundColor Cyan
    ...
    $evidenceJsonPath = Join-Path $EvidenceRoot 'B10-local-validation.json'
    ...
    $baseName = "FinScope_local_evidence_B10_${RunStamp}" + $(if ($overallPass) { '' } else { '_FAILED' })
