...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$WorkRoot = 'C:\FS\B07r2v1\work',
...
$CandidateLogicalName = 'FS_B07_r2.zip'
$CandidateSidecarLogicalName = 'FS_B07_r2.zip.sha256'
$RunnerLogicalName = 'Run-FinScope-BatchValidation_B07_r2_v1.ps1'
$RunnerSidecarLogicalName = 'Run-FinScope-BatchValidation_B07_r2_v1.ps1.sha256'
$ExpectedRootDirectory = 'FinScope_v0.21.4'
$ExpectedTasksSha256 = '94d6983807f00ea7e1aacf960790c5df9ec93313dd2b5f5549acb2cb0f301978'
$ExpectedSpecifyTreeSha256 = 'e06c8fbab523b824c144bb22b616001a3a4e810bb9daaa793e84d5cbb77c2c09'
$ExpectedSourceBaselineSha256 = '45526daf30092888bdba5333526e6806d22c12d986ef51ab1c31a4f68b9a321d'
...
function Assert-ShortWorkRoot {
    ...
    if ($fullPath -notmatch '^[A-Za-z]:\\FS\\[^\\]+\\work(?:\\)?$') {
        throw "WORK_ROOT_NOT_SHORT_OR_UNSAFE: use a path like C:\FS\B07r2v1\work; actual=${fullPath}"
    }
...
function Assert-PreNpmExecutability {
    ...
    foreach ($configPath in @('tsconfig.json', 'vitest.config.ts', 'playwright.config.ts')) {
    ...
    if ([bool]$Batch.localValidation.browserRequired -and $browserCommand.Count -ne 1) {
    ...
    foreach ($lockedDependency in @('node_modules/@playwright/test','node_modules/playwright-core','node_modules/vitest')) {
...
function Assert-ControlState {
    ...
    $batchPath = Join-Path $ProjectRoot 'implementation-control\batches\B07.json'
    ...
    if ([string]$batch.status -ne 'LOCAL_VALIDATION_REQUIRED') { throw 'B07_BATCH_FILE_STATUS_MISMATCH' }
    ...
    if ($lockEntry.Count -ne 1 -or [string]$lockEntry[0].sha256 -ne $batchActualHash) {
        throw "TASK_MIRROR_MISMATCH: B07 batch lock does not match ${batchActualHash}"
    }
...
function Invoke-RunnerQualificationSelfTests {
    ...
    $selfRoot = Join-Path ([System.IO.Path]::GetTempPath()) "FinScope-B07-runner-selftest-$([Guid]::NewGuid().ToString('N'))"
    New-Item -ItemType Directory -Path $selfRoot -Force | Out-Null
    try {
        $valueShapes = Invoke-ValueShapeSelfTests
        $safeWorkRoot = Assert-ShortWorkRoot 'C:\FS\B07r2v1\work'
        ...
        $validInspection = Get-ArchiveInspection $validZip 'C:\FS\B07-selftest\work'
        ...
        $rootNegative = Assert-ExpectedFailure -CaseId 'wrong-root' -ExpectedPattern 'ARCHIVE_PREFLIGHT_FAILED' -Action { [void](Assert-ArchiveInspection (Get-ArchiveInspection $wrongRootZip 'C:\FS\B07-selftest\work')) }

        $traversalZip = New-SyntheticZip (Join-Path $selfRoot 'traversal.zip') ([ordered]@{ "${ExpectedRootDirectory}/../escape.txt"='invalid' })
        $traversalNegative = Assert-ExpectedFailure -CaseId 'traversal' -ExpectedPattern 'ARCHIVE_PREFLIGHT_FAILED' -Action { [void](Assert-ArchiveInspection (Get-ArchiveInspection $traversalZip 'C:\FS\B07-selftest\work')) }

        $evidenceRoot = Join-Path $selfRoot 'evidence'
        New-Item -ItemType Directory -Path $evidenceRoot -Force | Out-Null
        foreach ($name in @('B07-local-validation.json','VERIFICATION_SUMMARY.json')) {
...
function New-FailureDiagnostic {
    ...
    try {
        ...
        $root = Join-Path $OutputDirectory "FinScope_runner_diagnostic_B07_${RunStamp}_FAILED"
...
try {
    ...
    if ($SelfTestOnly) {
        $selfTestPath = Join-Path $ResolvedInput "FinScope_runner_selftest_B07_${RunStamp}.json"
        ...
        Write-Host "Runner B07 r2 v1 SelfTest PASS (sin candidato ni npm): ${selfTestPath}" -ForegroundColor Green
        exit 0
    }
    ...
    $preflight = [ordered]@{
        ...
        hashes = [ordered]@{ tasksSha256=$tasksHash; specifyTreeSha256=$specifyHash; B07Sha256=$control.BatchHash; extractedTreeSha256=$initialTreeSha256 }
        packageHygiene = $packageHygiene
        authorityFiles = $authorityFiles
        executability = $executability
        state = [ordered]@{ implementationStatus=[string]$control.State.implementationStatus; activeBatchId=[string]$control.State.activeBatchId; nextAuthorizedBatchId=[string]$control.State.nextAuthorizedBatchId; B06=[string]$control.State.batchStatus.B06; B07=[string]$control.State.batchStatus.B07; B08=[string]$control.State.batchStatus.B08; convergenceAuthorized=[bool]$control.State.phaseGate.convergenceAuthorized }
    }
    $PreflightReportPath = Join-Path $ResolvedInput "FinScope_preflight_B07_${RunStamp}.json"
    ...
    Write-Host "Preflight B07 r2 v1 PASS (sin npm): ${PreflightReportPath}" -ForegroundColor Green
    if ($PreflightOnly) { exit 0 }

    $EvidenceRoot = Join-Path $resolvedWorkRoot "FinScope_local_evidence_B07_${RunStamp}"
    ...
    try {
        foreach ($entry in @($control.Batch.localValidation.commands)) {
            ...
            Write-Host "[B07] Ejecutando: $($entry.command)" -ForegroundColor Cyan
    ...
    $evidenceJsonPath = Join-Path $EvidenceRoot 'B07-local-validation.json'
    ...
    $baseName = "FinScope_local_evidence_B07_${RunStamp}" + $(if ($overallPass) { '' } else { '_FAILED' })
