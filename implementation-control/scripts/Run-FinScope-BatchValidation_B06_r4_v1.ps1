...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$WorkRoot = 'C:\FS\B06r4v1\work',
...
$CandidateLogicalName = 'FS_B06_r4.zip'
$CandidateSidecarLogicalName = 'FS_B06_r4.zip.sha256'
$RunnerLogicalName = 'Run-FinScope-BatchValidation_B06_r4_v1.ps1'
$RunnerSidecarLogicalName = 'Run-FinScope-BatchValidation_B06_r4_v1.ps1.sha256'
$ExpectedRootDirectory = 'FinScope_v0.21.4'
$ExpectedTasksSha256 = '36dd42fdd7a6d045a142cc9a7ddfdadbccd4e0d39ab3032da5d090e6e349062b'
$ExpectedSpecifyTreeSha256 = 'e06c8fbab523b824c144bb22b616001a3a4e810bb9daaa793e84d5cbb77c2c09'
$ExpectedSourceBaselineSha256 = '603cc07bb65f69483c85266aa91ee9b2cf681fe809c98456eea482a71cb78db5'
...
function Assert-ShortWorkRoot {
    ...
    if ($fullPath -notmatch '^[A-Za-z]:\\FS\\[^\\]+\\work(?:\\)?$') {
        throw "WORK_ROOT_NOT_SHORT_OR_UNSAFE: use a path like C:\FS\B06r4v1\work; actual=${fullPath}"
    }
...
function Assert-ControlState {
    ...
    $batchPath = Join-Path $ProjectRoot 'implementation-control\batches\B06.json'
    ...
    if ([string]$state.batchStatus.B05 -ne 'COMPLETED' -or [string]$state.batchStatus.B06 -ne 'LOCAL_VALIDATION_REQUIRED') {
        throw "BATCH_STATUS_MISMATCH: B05=$($state.batchStatus.B05); B06=$($state.batchStatus.B06)"
    }
    if ([string]$batch.status -ne 'LOCAL_VALIDATION_REQUIRED') { throw 'B06_BATCH_FILE_STATUS_MISMATCH' }
    ...
    if ($lockEntry.Count -ne 1 -or [string]$lockEntry[0].sha256 -ne $batchActualHash) {
        throw "TASK_MIRROR_MISMATCH: B06 batch lock does not match ${batchActualHash}"
    }
    foreach ($taskId in @('T024','T025','T026','T027','T028','T030')) {
    ...
    if ([string]$state.taskStatus.T029 -ne 'PENDING' -or [string]$state.batchStatus.B07 -ne 'PENDING') {
        throw 'FUTURE_SCOPE_STATUS_CHANGED'
    }
...
function New-FailureDiagnostic {
    ...
    try {
        ...
        $root = Join-Path $OutputDirectory "FinScope_runner_diagnostic_B06_${RunStamp}_FAILED"
...
try {
    ...
    $preflight = [ordered]@{
        ...
        hashes = [ordered]@{ tasksSha256=$tasksHash; specifyTreeSha256=$specifyHash; B06Sha256=$control.BatchHash; extractedTreeSha256=$initialTreeSha256 }
        packageHygiene = $packageHygiene
        executability = $executability
        state = [ordered]@{ implementationStatus=[string]$control.State.implementationStatus; activeBatchId=[string]$control.State.activeBatchId; nextAuthorizedBatchId=[string]$control.State.nextAuthorizedBatchId; B05=[string]$control.State.batchStatus.B05; B06=[string]$control.State.batchStatus.B06; B07=[string]$control.State.batchStatus.B07; convergenceAuthorized=[bool]$control.State.phaseGate.convergenceAuthorized }
    }
    $PreflightReportPath = Join-Path $ResolvedInput "FinScope_preflight_B06_${RunStamp}.json"
    ...
    Write-Host "Preflight B06 r4 PASS (sin npm): ${PreflightReportPath}" -ForegroundColor Green
    if ($PreflightOnly) { exit 0 }

    $EvidenceRoot = Join-Path $resolvedWorkRoot "FinScope_local_evidence_B06_${RunStamp}"
    ...
    try {
        foreach ($entry in @($control.Batch.localValidation.commands)) {
            ...
            Write-Host "[B06] Ejecutando: $($entry.command)" -ForegroundColor Cyan
    ...
    $evidenceJsonPath = Join-Path $EvidenceRoot 'B06-local-validation.json'
    Write-Utf8Json $evidenceJsonPath $evidence
    Write-Utf8Json (Join-Path $runnerArtifactRoot 'runtime-and-parser-self-tests.json') ([ordered]@{ schemaVersion='1.0.0'; status='PASS'; astTokenCount=$astTokenCount; cultureAndJsonSelfTests='PASS'; parserSelfTests=$parserSelfTests; runnerSha256=$AuthenticatedRunnerSha256 })
    ...
    $baseName = "FinScope_local_evidence_B06_${RunStamp}" + $(if ($overallPass) { '' } else { '_FAILED' })
